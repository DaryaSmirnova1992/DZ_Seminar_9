...
def get_value(flag):
    ...
    if flag == 0:
        ...
        while (chck):
            chck = True
            if numb_type == 'r':
                numb_value = input('''Формат вещественных чисел с использованием "." (пример, 5.35); 
                в виде дроби в формате "[целое часть] пробел [числитель] / [знаменатель] (пример, 3 5/6).
                Введите число:   ''')
            ...
            if numb_type == 'c':
                numb_value = input(
                    'Формат комплексных чисел: [действительная часть] + [мнимая часть]j (пример, 6-5j).  Введите число:')
...
def get_oper(chck):
    ...
    chck = True
    while (chck):
        numb_oper = input('введите оператор (+, -, *, /): ')
